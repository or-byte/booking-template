...
export const bodyTemplate = (pkg: Package) => {
  let status: keyof typeof PackageStatus = PackageStatus.CREATED;
  let updatedBy = pkg.createdBy.fullName;

  if (pkg.approvedBy) {
    status = PackageStatus.APPROVED;
    updatedBy = pkg.approvedBy.fullName;
  } else if (pkg.reviewedBy) {
    status = PackageStatus.REVIEWED;
    updatedBy = pkg.reviewedBy.fullName;
  } else if (pkg.updatedAt && pkg.updatedAt !== pkg.createdAt) {
    status = PackageStatus.MODIFIED;
    updatedBy = pkg.updatedBy.fullName;
  } else {
    status = PackageStatus.CREATED;
    updatedBy = pkg.createdBy.fullName;
  }
